                ...
                <div class="col-lg-5 mb-all-40">
                    ...
                    <div class="tab-content">
                        <div id="thumb{{ $product->id }}"
                            class="tab-pane fade show active d-flex align-items-center aspect-ratio d-flex justify-content-center">
                            <a data-fancybox="images" id="link-image-main"
                                href="{{ '.' . Storage::url($product->image) }}"><img id="image-main" class="w-100"
                                    src="{{ '.' . Storage::url($product->image) }}" alt="product-view"></a>
                        </div>
                        @foreach ($product->galleries as $gallery)
                            <div id="thumbgl{{ $gallery->id }}"
                                class="tab-pane fade d-flex align-items-center aspect-ratio d-flex justify-content-center">
                                <a data-fancybox="images" href="{{ '.' . Storage::url($gallery->path) }}"><img
                                        src="{{ '.' . Storage::url($gallery->path) }}" class="w-100"
                                        alt="product-view"></a>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="product-thumbnail">
                        <div class="thumb-menu owl-carousel nav tabs-area" role="tablist">
                            <a class="active d-flex align-items-center aspect-ratio" data-bs-toggle="tab"
                                href="#thumb{{ $product->id }}"><img src="{{ '.' . Storage::url($product->image) }}"
                        ...
                        <div class="thubnail-desc fix">
                            ...
                            <div class="pro-price mtb-10">
                                <p class="d-flex align-items-center"><span class="price"
                                        id="price">{{ number_format($product->product_variants_min_price, 0, '', '.') }}
                                        vnđ - {{ number_format($product->product_variants_max_price, 0, '', '.') }}
                                        vnđ</span><span id="sale"></span></p>
                            </div>
                                ...
                                <div class="product-options">
                                    @foreach ($product->productVariants->unique('color_id') as $productVariant)
                                        <div class="form-check ps-0">
                                            <input class="form-check-input form-check-input2" type="radio"
                                                name="color" id="color{{ $productVariant->color->id }}"
                                                value="{{ $productVariant->color->id }}" required>
                                            <label class="form-check-label ms-0 me-2 text-nowrap"
                                                for="color{{ $productVariant->color->id }}">{{ $productVariant->color->name }}</label>
                                        </div>
                                    @endforeach
                                </div>
                                @if ($errors->has('color'))
                            ...
                            <div class="product-size mb-20 clearfix">
                                ...
                                <div class="product-options">
                                    @foreach ($product->productVariants->unique('ssd_id') as $productVariant)
                                        <div class="form-check ps-0">
                                            <input class="form-check-input form-check-input1" type="radio"
                                                name="ssd" id="ssd{{ $productVariant->ssd->id }}"
                                                value="{{ $productVariant->ssd->id }}" required>
                                            <label class="form-check-label ms-0 me-2 text-nowrap"
                                                for="ssd{{ $productVariant->ssd->id }}">{{ $productVariant->ssd->name }}</label>
                                        </div>
                                    @endforeach
                                </div>
                                @if ($errors->has('ssd'))
                            ...
                            <div class="box-quantity d-flex my-4">
                                ...
                                <input class="quantity mr-40" name="quantity" type="number" min="1"
                                    value="1">
                            ...
                            <div class="pro-ref mt-15">
                                <label><b>Số lượng có sẵn:</b> <span
                                        id="result-quantity">{{ $product->product_variants_sum_quantity }}</span></label>
                            </div>
                        </div>
                    </form>
                                ...
                                <div class="row mb-4">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="star" id="star" class="form-select">
                                            ...
                                            <option value="5" {{ request('star') == 5 ? 'selected' : '' }}>5 sao
                                            </option>
                                            <option value="4" {{ request('star') == 4 ? 'selected' : '' }}>4 sao
                                            </option>
                                            <option value="3" {{ request('star') == 3 ? 'selected' : '' }}>3 sao
                                            </option>
                                            <option value="2" {{ request('star') == 2 ? 'selected' : '' }}>2 sao
                                            </option>
                                            <option value="1" {{ request('star') == 1 ? 'selected' : '' }}>1 sao
                                            </option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="sort" id="sort" class="form-select">
                                            ...
                                            <option value="oldest"
                                                {{ request('sort') == 'oldest' ? 'selected' : '' }}>Cũ nhất</option>
                                        </select>
                                    </div>
                    ...
                    <div class="pro-content">
                        ...
                        <div class="pro-actions">
                            <div class="actions-primary">
                                <a href="cart.html" class="px-1" data-bs-toggle="tooltip" data-bs-placement="top"
    ...
    <script>
        ...
        function checkBothSelected() {
            ...
            if (colorsChecked) {
                ...
                let validSSDs = productVariants.filter(variant => variant.color_id == colorValue.value && variant.quantity >
                        0)
            ...
            if (ssdsChecked) {
                ...
                let validColors = productVariants.filter(variant => variant.ssd_id == ssdValue.value && variant.quantity >
                        0)
            ...
            if (colorsChecked && ssdsChecked) {
                productVariants.forEach(variant => {
                    if (ssdValue.value == variant.ssd_id && colorValue.value == variant.color_id) {
                        ...
                        price.innerHTML = '<del class="prev-price me-3">' + variant.listed_price.toLocaleString(
                            'de-DE') + ' vnđ</del>' + variant.price.toLocaleString('de-DE') + ' vnđ';
